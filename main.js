...
class Binance {
    constructor() {
        ...
        this.headers = {
            ...
            "Accept-Language": "jv-ID,jv;q=0.9,en-US;q=0.8,en;q=0.7",
    ...
    async callBinanceAPI(queryString) {
        ...
        try {
            ...
            if (accessTokenResponse.data.code !== "000000" || !accessTokenResponse.data.success) {
                throw new Error(`Gagal entuk token akses: ${accessTokenResponse.data.message}`);
            }
            ...
            if (userInfoResponse.data.code !== "000000" || !userInfoResponse.data.success) {
                throw new Error(`Gagal entuk informasi pengguna: ${userInfoResponse.data.message}`);
            }
    ...
    async startGame(accessToken) {
        try {
            ...
            if (response.data.code === '000000') {
                this.log("Miwa game sukses", 'success');
                return true;
            }

            if (response.data.code === '116002') {
                this.log("Ora cukup tiket main!", 'warning');
            } else {
                this.log("Kesalahan nalika miwa game!", 'error');
            }

            return false;
        } catch (error) {
            this.log(`Ora bisa miwa game: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    async completeGame(accessToken) {
        ...
        try {
            ...
            if (data.success) {
                this.log(`Rampung game sukses | Entuk ${this.game.log} poin`, 'custom');
                return true;
            } else {
    ...
    async playGameIfTicketsAvailable(queryString, accountIndex, firstName) {
        console.log(`========== Akun ${accountIndex} | ${firstName.green} ==========`);
        ...
        this.log(`Tiket sing ana: ${availableTickets}`);
        await this.completeTasks(accessToken);
        while (availableTickets > 0) {
            this.log(`Miwa game karo ${availableTickets} tiket sing kasedhiya`, 'info');
            ...
            if (await this.startGame(accessToken)) {
                if (await this.getGameData()) {
                    ...
                    if (await this.completeGame(accessToken)) {
                        availableTickets--;
                        this.log(`Tiket sing tersisa: ${availableTickets}`, 'info');
                    } else {
                        break;
                    }
                } else {
                ...
                this.log("Ora bisa miwa game", 'error');
                break;
            }
        ...
        if (availableTickets === 0) {
            this.log("Kabeh tiket wis digunakake", 'success');
        }
    }
